...
create table SOAPP_SUPPLIER_CONTACTS (
    id                   number generated by default on null as identity
                         constraint soapp_supplier_contacts_id_pk primary key,
    CONTACT_ID           number,
    PROFILE_ID           number,
...
-- triggers
create or replace trigger soapp_supplier_contacts_biu
    before insert or update
    on soapp_supplier_contacts
    for each row
begin
    if inserting then
...
end soapp_supplier_contacts_biu;
/


-- APIs
create or replace package soapp_supplier_contacts_api
is
...
end soapp_supplier_contacts_api;
/

create or replace package body soapp_supplier_contacts_api
is
    ...
    is 
    begin 
        for c1 in (select * from soapp_supplier_contacts where id = p_id) loop 
    ...
    is 
    begin 
        insert into soapp_supplier_contacts ( 
            id,
            contact_id,
            profile_id,
            first_name,
    ...
    is 
    begin 
        update  soapp_supplier_contacts set 
            id = p_id,
...
,
    ...
    is
    begin
        delete from soapp_supplier_contacts where id = p_id;
    end delete_row;
end soapp_supplier_contacts_api;
/

-- Grant select, insert, update, delete access to GSAPEX schema for SOAPP_SUPPLIER_CONTACTS table
GRANT SELECT, INSERT, UPDATE, DELETE ON GSSOAPP.SOAPP_SUPPLIER_CONTACTS TO gsapex;
/
-- Grant execute privilege on soapp_supplier_contacts_api api to GSAPEX schema
GRANT EXECUTE ON GSSOAPP.soapp_supplier_contacts_api TO gsapex;
/
-- Create synonyms for easier access in GSAPEX schema
CREATE OR REPLACE SYNONYM gsapex.SOAPP_SUPPLIER_CONTACTS FOR GSSOAPP.SOAPP_SUPPLIER_CONTACTS;
/
CREATE OR REPLACE SYNONYM gsapex.soapp_supplier_contacts_api FOR GSSOAPP.soapp_supplier_contacts_api;
-- Generated by Quick SQL 1.2.9 5/30/2024, 1:30:41 PM

/*
SOAPP_SUPPLIER_CONTACTS
    CONTACT_ID num,
    PROFILE_ID num,
    FIRST_NAME VC(100),
    LAST_NAME VC(100),
