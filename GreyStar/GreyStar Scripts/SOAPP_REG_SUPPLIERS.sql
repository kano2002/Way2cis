...
create table SOAPP_REG_SUPPLIERS (
...
-- triggers
create or replace trigger soapp_reg_suppliers_biu
    before insert or update
    on soapp_reg_suppliers
    for each row
begin
    if inserting then
...
end soapp_reg_suppliers_biu;
/


-- APIs
create or replace package soapp_reg_suppliers_api
is
...
end soapp_reg_suppliers_api;
/

create or replace package body soapp_reg_suppliers_api
is
    ...
    is 
    begin 
        for c1 in (select * from soapp_reg_suppliers where id = p_id) loop 
    ...
    is 
    begin 
        insert into soapp_reg_suppliers ( 
            id,
            reg_request_id,
            vendor_id,
            exempt_flag,
    ...
    is 
    begin 
        update  soapp_reg_suppliers set 
            id = p_id,
...
,
    ...
    is
    begin
        delete from soapp_reg_suppliers where id = p_id;
    end delete_row;
end soapp_reg_suppliers_api;
/

-- Grant select, insert, update, delete access to GSAPEX schema for SOAPP_REG_SUPPLIERS table
GRANT SELECT, INSERT, UPDATE, DELETE ON GSSOAPP.SOAPP_REG_SUPPLIERS TO gsapex;
/
-- Grant execute privilege on soapp_reg_suppliers_api api to GSAPEX schema
GRANT EXECUTE ON GSSOAPP.soapp_reg_suppliers_api TO gsapex;

-- Generated by Quick SQL 1.2.9 5/28/2024, 9:34:49 PM
