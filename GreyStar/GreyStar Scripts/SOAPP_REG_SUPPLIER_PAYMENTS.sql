...
create table SOAPP_REG_SUPPLIER_PAYMENTS (
    id                       number generated by default on null as identity
                             constraint soapp_reg_supplier_payments_id_pk primary key,
    PAYMENT_ID               number,
    PROFILE_ID               number,
...
-- triggers
create or replace trigger soapp_reg_supplier_payments_biu
    before insert or update
    on soapp_reg_supplier_payments
    for each row
begin
    if inserting then
...
end soapp_reg_supplier_payments_biu;
/


-- APIs
create or replace package soapp_reg_supplier_payments_api
is
...
end soapp_reg_supplier_payments_api;
/

create or replace package body soapp_reg_supplier_payments_api
is
    ...
    is 
    begin 
        for c1 in (select * from soapp_reg_supplier_payments where id = p_id) loop 
    ...
    is 
    begin 
        insert into soapp_reg_supplier_payments ( 
            id,
            payment_id,
            profile_id,
            invoice_currency,
    ...
    is 
    begin 
        update  soapp_reg_supplier_payments set 
            id = p_id,
...
,
    ...
    is
    begin
        delete from soapp_reg_supplier_payments where id = p_id;
    end delete_row;
end soapp_reg_supplier_payments_api;
/
 -- Grant select, insert, update, delete access to GSAPEX schema for SOAPP_REG_SUPPLIER_PAYMENTS table
GRANT SELECT, INSERT, UPDATE, DELETE ON GSSOAPP.SOAPP_REG_SUPPLIER_PAYMENTS TO gsapex;
/
-- Grant execute privilege on soapp_reg_supplier_payments_api api to GSAPEX schema
GRANT EXECUTE ON GSSOAPP.soapp_reg_supplier_payments_api TO gsapex;
/
-- Create synonyms for easier access in GSAPEX schema
CREATE OR REPLACE SYNONYM gsapex.SOAPP_REG_SUPPLIER_PAYMENTS FOR GSSOAPP.SOAPP_REG_SUPPLIER_PAYMENTS;
/
CREATE OR REPLACE SYNONYM gsapex.soapp_reg_supplier_payments_api FOR GSSOAPP.soapp_reg_supplier_payments_api;

-- Generated by Quick SQL 1.2.9 5/28/2024, 5:38:23 PM

/*
SOAPP_REG_SUPPLIER_PAYMENTS
