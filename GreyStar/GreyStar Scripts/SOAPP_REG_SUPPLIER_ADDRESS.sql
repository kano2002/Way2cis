...
create table SOAPP_REG_SUPPLIER_ADDRESS (
    id                   number generated by default on null as identity
                         constraint soapp_reg_supplier_address_id_pk primary key,
    ADDRESS_ID           number,
    VENDOR_ID            number,
    PROFILE_ID           number,
...
-- triggers
create or replace trigger soapp_reg_supplier_address_biu
    before insert or update
    on soapp_reg_supplier_address
    for each row
begin
    if inserting then
...
end soapp_reg_supplier_address_biu;
/


-- APIs
create or replace package soapp_reg_supplier_address_api
is
...
end soapp_reg_supplier_address_api;
/

create or replace package body soapp_reg_supplier_address_api
is
    ...
    is 
    begin 
        for c1 in (select * from soapp_reg_supplier_address where id = p_id) loop 
    ...
    is 
    begin 
        insert into soapp_reg_supplier_address ( 
            id,
            address_id,
            vendor_id,
            profile_id,
    ...
    is 
    begin 
        update  soapp_reg_supplier_address set 
            id = p_id,
...
,
    ...
    is
    begin
        delete from soapp_reg_supplier_address where id = p_id;
    end delete_row;
end soapp_reg_supplier_address_api;
/

-- Grant select, insert, update, delete access to GSAPEX schema for SOAPP_REG_SUPPLIER_ADDRESS table
GRANT SELECT, INSERT, UPDATE, DELETE ON GSSOAPP.SOAPP_REG_SUPPLIER_ADDRESS TO gsapex;
/
-- Grant execute privilege on soapp_reg_supplier_address_api api to GSAPEX schema
GRANT EXECUTE ON GSSOAPP.soapp_reg_supplier_address_api TO gsapex;
/
-- Create synonyms for easier access in GSAPEX schema
CREATE OR REPLACE SYNONYM gsapex.SOAPP_REG_SUPPLIER_ADDRESS FOR GSSOAPP.SOAPP_REG_SUPPLIER_ADDRESS;
/
CREATE OR REPLACE SYNONYM gsapex.soapp_reg_supplier_address_api FOR GSSOAPP.soapp_reg_supplier_address_api;
-- Generated by Quick SQL 1.2.9 5/28/2024, 5:26:56 PM

/*
SOAPP_REG_SUPPLIER_ADDRESS
    ADDRESS_ID	            NUMBER,	
    VENDOR_ID	            NUMBER,	
    PROFILE_ID	            NUMBER,	
    REG_REQUEST_ID	        NUMBER,	
