...
create table SOAPP_SUPPLIER_PROFILES (
    id                   number generated by default on null as identity
                         constraint soapp_supplier_profiles_id_pk primary key,
    PROFILE_ID           number,
...
-- triggers
create or replace trigger soapp_supplier_profiles_biu
    before insert or update
    on soapp_supplier_profiles
    for each row
begin
    if inserting then
...
end soapp_supplier_profiles_biu;
/


-- APIs
create or replace package soapp_supplier_profiles_api
is
...
end soapp_supplier_profiles_api;
/

create or replace package body soapp_supplier_profiles_api
is
    ...
    is 
    begin 
        for c1 in (select * from soapp_supplier_profiles where id = p_id) loop 
    ...
    is 
    begin 
        insert into soapp_supplier_profiles ( 
            id,
            profile_id,
            supplier_name,
            language,
    ...
    is 
    begin 
        update  soapp_supplier_profiles set 
            id = p_id,
...
,
    ...
    is
    begin
        delete from soapp_supplier_profiles where id = p_id;
    end delete_row;
end soapp_supplier_profiles_api;
/
 -- Grant select, insert, update, delete access to GSAPEX schema for SOAPP_SUPPLIER_PROFILES table
GRANT SELECT, INSERT, UPDATE, DELETE ON GSSOAPP.SOAPP_SUPPLIER_PROFILES TO gsapex;
/
-- Grant execute privilege on soapp_supplier_profiles_api api to GSAPEX schema
GRANT EXECUTE ON GSSOAPP.soapp_supplier_profiles_api TO gsapex;
/
-- Create synonyms for easier access in GSAPEX schema
CREATE OR REPLACE SYNONYM gsapex.SOAPP_SUPPLIER_PROFILES FOR GSSOAPP.SOAPP_SUPPLIER_PROFILES;
/
CREATE OR REPLACE SYNONYM gsapex.soapp_supplier_profiles_api FOR GSSOAPP.soapp_supplier_profiles_api;
-- Generated by Quick SQL 1.2.9 5/28/2024, 5:14:56 PM

/*
SOAPP_SUPPLIER_PROFILES
    PROFILE_ID num,
