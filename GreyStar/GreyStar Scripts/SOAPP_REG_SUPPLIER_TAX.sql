...
create table SOAPP_REG_SUPPLIER_TAX (
    id                   number generated by default on null as identity
                         constraint soapp_reg_supplier_tax_id_pk primary key,
    TAX_ID               number,
...
-- triggers
create or replace trigger soapp_reg_supplier_tax_biu
    before insert or update
    on soapp_reg_supplier_tax
    for each row
begin
    if inserting then
...
end soapp_reg_supplier_tax_biu;
/


-- APIs
create or replace package soapp_reg_supplier_tax_api
is
...
end soapp_reg_supplier_tax_api;
/

create or replace package body soapp_reg_supplier_tax_api
is
    ...
    is 
    begin 
        for c1 in (select * from soapp_reg_supplier_tax where id = p_id) loop 
            p_tax_id := c1.tax_id;
    ...
    is 
    begin 
        insert into soapp_reg_supplier_tax ( 
            id,
            tax_id,
            company_name,
            tax_payer_id,
    ...
    is 
    begin 
        update  soapp_reg_supplier_tax set 
            id = p_id,
            tax_id = p_tax_id
,
...
,
    ...
    is
    begin
        delete from soapp_reg_supplier_tax where id = p_id;
    end delete_row;
end soapp_reg_supplier_tax_api;
/
 -- Grant select, insert, update, delete access to GSAPEX schema for SOAPP_REG_SUPPLIER_TAX table
GRANT SELECT, INSERT, UPDATE, DELETE ON GSSOAPP.SOAPP_REG_SUPPLIER_TAX TO gsapex;
/
-- Grant execute privilege on soapp_reg_supplier_tax_api api to GSAPEX schema
GRANT EXECUTE ON GSSOAPP.soapp_reg_supplier_tax_api TO gsapex;
/
-- Create synonyms for easier access in GSAPEX schema
CREATE OR REPLACE SYNONYM gsapex.SOAPP_REG_SUPPLIER_TAX FOR GSSOAPP.SOAPP_REG_SUPPLIER_TAX;
/
CREATE OR REPLACE SYNONYM gsapex.soapp_reg_supplier_tax_api FOR GSSOAPP.soapp_reg_supplier_tax_api;
-- Generated by Quick SQL 1.2.9 5/28/2024, 5:41:59 PM

/*
SOAPP_REG_SUPPLIER_TAX
    TAX_ID				NUMBER	
    COMPANY_NAME		vc(100)
    TAX_PAYER_ID		vc(100)
    ATTACHMENT_ID		NUMBER	
